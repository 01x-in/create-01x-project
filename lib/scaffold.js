...
function resolveTargetDirectory({
  ...
  const resolvedFolderName = typeof folderName === 'string' && folderName.trim().length > 0
    ? folderName
    : suggestedFolderName
  const targetDirectoryMode = useCurrentDirectory ? 'current' : 'custom'
  const targetDir = useCurrentDirectory
    ? cwd
    : path.isAbsolute(resolvedFolderName)
      ? resolvedFolderName
      : path.resolve(cwd, resolvedFolderName)
...
function renderClaudeAgent(agent, runtimeConfig) {
  ...
  const instructions = `${getClaudeContextModeHint(agent.baseName)}${getRuntimeSpecificInstructions(agent, runtimeConfig)}`
...
function renderContextModeSetup(runtime) {
  if (runtime === 'claude') {
    return `Recommended setup for Claude Code:
- Install via plugin marketplace:
  - \`/plugin marketplace add mksglu/context-mode\`
  - \`/plugin install context-mode@context-mode\`
- Verify in Claude Code with: \`/context-mode:ctx-doctor\`
- MCP-only fallback: \`claude mcp add context-mode -- npx -y context-mode\`
`
  }
...
function renderContextModeUsageNotes() {
  return `If context-mode is available in this runtime, prefer it for large-output operations:
- reading many files or very large files
- build, test, lint, and typecheck output
- large diffs, PR comment batches, and API responses
- web fetches, snapshots, logs, and other bulky inspection tasks

Never block the workflow on context-mode. If it is not installed, fall back to the normal runtime tools and keep outputs tight.
`
}

function renderClaudeContextModeSection(runtime) {
  if (runtime !== 'claude') {
    return ''
  }

  return `## Context Efficiency — Optional but Recommended

${renderContextModeUsageNotes()}
${renderContextModeSetup(runtime)}
`
}

function getClaudeContextModeHint(agentBaseName) {
  ...
  if (!supportedAgents.has(agentBaseName)) {
    return ''
  }

  return `If context-mode is installed in Claude Code, prefer it for large reads,
large logs, snapshots, diffs, and bulky inspection output. Do not block on it
...
${renderClaudeContextModeSection(runtime)}
...
3. README Agent      → rewrites the root README.md from the approved planning docs
4. Build Loop        → build → test → review → fix → repeat per story
5. UI/UX Gate        → validates the built frontend against design-spec UI Assertions via PinchTab
6. PR Review Loop    → opens PR, fixes review comments, replies and resolves threads
...
> Starter README. After planning approval, readme-agent will replace this with project-specific content.
...
${runtime === 'claude'
    ? `## Optional — Install Context Mode

Context Mode is recommended for long sessions and large-output tasks in Claude Code.
It keeps big reads, logs, snapshots, and API responses out of the model context window.
...
The workflow creates 5 planning docs, runs a review pass, refreshes \`README.md\`, then stops for human approval:
...
| readme-agent | post-review | Rewrites the root README.md from the approved planning docs |
...
function initialiseGit({ targetDir, preexistingEntries }) {
  ...
  try {
    ...
    if (result.initialized && preexistingEntries.length === 0) {
